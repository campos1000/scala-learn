...
import FloatOps._
  ...
  test("Empty.insert(b) should return a Leaf with only that body") {
    ...
    val d = new Body(3f, 47f, 44f, 0f, 0f)
    ...
    val insertedNew = inserted.insert(d)
    insertedNew match {
      case Fork(quadNw, quadNe, quadSw, quadSe) =>
        quadNw match {
          case Leaf(centerX, centerY, size, bodies) =>
            assert(true, s"$centerX should be 51f")
          case _ =>
            fail(s"HOHOHOHOHOH, was $insertedNew")
        }
      case _ =>
        fail("Empty.insert() BLAHBLAHBLAH")
    }
  }
  ...
  test("'SectorMatrix.+=' should add a body at (25,47) to the correct bucket of a sector matrix of size 96") {
    ...
    val test = sm.combine(sm1)
    val test1 = test(2,3).size
    //assert(false, s"$test1")
  }
