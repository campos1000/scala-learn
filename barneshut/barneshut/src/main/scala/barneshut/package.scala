...
package object barneshut {
  ...
  case class Empty(centerX: Float, centerY: Float, size: Float) extends Quad {
    ...
    def insert(b: Body): Quad = new Leaf(centerX, centerY, size, Array[Body](b))
  }
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if (b.x <= nw.centerX + nw.size / 2) {
        if (b.y <= nw.centerY + nw.size / 2) {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    extends Quad {
    val mass = bodies.foldLeft(0F)((acc, body) => acc + body.mass)
    val (massX, massY) = (
      (bodies.foldLeft(0F)((acc, body) => acc + body.x*body.mass)) / mass,
      (bodies.foldLeft(0F)((acc, body) => acc + body.y*body.mass)) / mass)
    ...
    def insert(b: Body): Quad = if (size <= minimumSize) {
      ...
      val EmptyFork = Fork(Empty(centerX - size/4, centerY - size/4, size / 2),
        Empty(size/4 + centerX, centerY - size/4, size / 2),
        Empty(centerX - size/4, size/4 + centerY, size / 2),
        Empty(size / 4 + centerX, size / 4 + centerY, size / 2))
      val newFork = bodies.foldLeft(EmptyFork)((fork, body) => fork.insert(body))
      newFork.insert(b)
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val yAdd = {
        val tempYAdd = ((b.y - boundaries.minY)/ sectorSize).toInt
        if (tempYAdd < sectorPrecision) tempYAdd else sectorPrecision - 1
      }
      val xAdd = {
        val tempXAdd = ((b.x - boundaries.minX) / sectorSize).toInt
        if (tempXAdd < sectorPrecision) tempXAdd else sectorPrecision - 1
      }
      matrix(yAdd * sectorPrecision + xAdd) += b
      this
    }
