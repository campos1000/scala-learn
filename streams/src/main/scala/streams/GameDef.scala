...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      (this.left, Left) :: (this.right, Right) :: (this.up, Up) :: (this.down, Down) :: List()
    }
     ...
     */
    def legalNeighbors: List[(Block, Move)] = this.neighbors.filter{ case (_1, _2) => terrain(_1.b1) && terrain(_1.b2)}
