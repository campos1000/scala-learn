...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var mapMeanToPoints = points.groupBy(point => findClosest(point, means))
    means.filter(mean => !mapMeanToPoints.contains(mean)).seq.foreach(mean => {
      mapMeanToPoints = mapMeanToPoints.updated(mean, GenSeq[Point]())
    })
    mapMeanToPoints
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zipWithIndex.aggregate(true)((checkResult, point) => checkResult && point._1.squareDistance(newMeans(point._2)) <= eta , _ && _)
  }
